...
async function handleLogin() {
  try {
    await store.fetchLogin(credentials.value)
  } catch (error) {
...
<template>
  <div class="login-box">
    ...
    <form @submit.prevent="handleLogin">
      ...
      <p>Nincs még fiókod?
        <router-link style="text-decoration: underline; color: white; font-weight: 700;" to="/register">Regisztrálj
          itt!
        </router-link>
      </p>
    </form>
...
.login-box .user-box input:focus~div,
.login-box .user-box input:valid~div
  top: -20px
  left: 0
  color: #fc2658
  font-size: 12px
